...
abstract class Model
{
    ...
    public function appointmentByPatient(int $id): bool
    {
        $query = "SELECT * FROM appointments WHERE patient_id = :id LIMIT 1";
    ...
    public function appointmentByDentist(int $id): bool
    {
        $query = "SELECT * FROM appointments WHERE dentist_id = :id LIMIT 1";
    ...
    {
        try {
            $query = "UPDATE {$this->table} SET status = 0 WHERE {$this->primaryKey} = :id";
            $stmt = Connect::getInstance()->prepare($query);
